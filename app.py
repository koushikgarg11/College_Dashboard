...
GRADE_C = {"S — Exceptional":"#6366f1","A — Great":"#06b6d4","B — Good":"#10b981","C — Fair":"#f59e0b"}
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:8px 0 16px 0;'>
      ...
      <div style='font-size:10px;color:#475569;margin-top:3px;'>India's Tier 1 Engineering Intelligence Vault</div>
    </div>""", unsafe_allow_html=True)
    ...
    st.markdown("---")
    pkg_min = int(df["Avg Pkg (LPA)"].min()); pkg_max = int(df["Avg Pkg (LPA)"].max())
    pkg_rng = st.slider("Avg Package (LPA)", pkg_min, pkg_max, (pkg_min, pkg_max))
    pl_min  = int(df["Placement %"].min());   pl_max  = int(df["Placement %"].max())
    pl_rng  = st.slider("Placement %", pl_min, pl_max, (pl_min, pl_max))
    st.markdown("---")
    st.markdown(f"""
...
fdf = fdf[(fdf["Avg Pkg (LPA)"] >= pkg_rng[0]) & (fdf["Avg Pkg (LPA)"] <= pkg_rng[1])]
fdf = fdf[(fdf["Placement %"]   >= pl_rng[0])  & (fdf["Placement %"]   <= pl_rng[1])]
...
if page == "🏠 Overview & Highlights":
    hero("NexusIQ — India's Tier 1 Engineering Intelligence Vault",
         "The definitive database of India's top 44 engineering colleges — placements, packages, recruiters, ROI & more",
         "OVERVIEW DASHBOARD")
    ...
    with c2:
        cat_agg = fdf.groupby("Category").size().reset_index(name="Count").sort_values("Count", ascending=False)
    ...
    tk  = fdf.nlargest(1,"Avg Pkg (LPA)").iloc[0]
    tr  = fdf.nlargest(1,"ROI Score").iloc[0]
    ...
    top10 = fdf.nlargest(10,"Avg Pkg (LPA)").sort_values("Avg Pkg (LPA)")
    fig6 = px.bar(top10, x="Avg Pkg (LPA)", y="College Name", orientation="h",
                  color="Tier", color_discrete_map=TIER_C,
                  text="Avg Pkg Range",
...
# ═══════════════════════════════════════════════════════════════
    ...
    with tabs[0]:
        ...
        with c2:
            fig3 = px.bar(
                t1.sort_values("Avg Pkg (LPA)", ascending=True),
    ...
    with tabs[1]:
        sec("Domestic vs International Package Comparison")
        pkg_df = t1.dropna(subset=["Intl Pkg (LPA)"]).copy()
        if len(pkg_df):
            fig4 = go.Figure()
            fig4.add_trace(go.Bar(
                name="Domestic Highest (LPA)",
                y=pkg_df["College Name"],
                x=pkg_df["Highest Pkg (LPA)"],
        ...
        sec("Avg Package Range vs Highest Package")
        fig5 = px.scatter(
            t1, x="Avg Pkg (LPA)", y="Highest Pkg (LPA)",
        ...
        pkg_show = t1[["College Name","Category","Avg Pkg","Highest Pkg","Avg Pkg (LPA)","Highest Pkg (LPA)","Intl Pkg (LPA)"]].sort_values("Avg Pkg (LPA)", ascending=False).reset_index(drop=True)
    ...
    with tabs[2]:
        ...
        with c1:
            ...
            fig6 = px.bar(state_t1.sort_values("Count", ascending=True),
        ...
        with c2:
            own_t1 = t1.groupby("Ownership Type").agg(
                Count=("College Name","count"),
                Avg_K=("Avg Pkg (LPA)","mean")
            ).reset_index()
...
# ═══════════════════════════════════════════════════════════════
    ...
    hero("State-wise College Distribution",
         "How India's premier engineering colleges are spread across 18 states",
         "GEO ANALYTICS")
    ...
    with tabs[1]:
        ...
        fig4 = px.bar(ss.sort_values("Avg_Package", ascending=False),
                      ...
                      text=ss.sort_values("Avg_Package", ascending=False)["Avg_Package"].apply(lambda x: f"₹{x:.1f}"),
...
# ═══════════════════════════════════════════════════════════════
    ...
    c4.metric("Highest Package",  f"₹{fdf['Highest Pkg (LPA)'].max():.0f} LPA")
    ...
    with tabs[0]:
        ...
        with c2:
            fig2 = px.histogram(fdf, x="Avg Pkg (LPA)", nbins=12,
                                color="Category", color_discrete_map=CAT_C,
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig3 = px.box(fdf, x="Tier", y="Avg Pkg (LPA)",
                          color="Tier", color_discrete_map=TIER_C,
        ...
        with c2:
            fig4 = px.box(fdf, x="Category", y="Avg Pkg (LPA)",
                          color="Category", color_discrete_map=CAT_C,
                          points="all", hover_name="College Name",
                          hover_data={"Avg Pkg Range": True},
    ...
    with tabs[2]:
        ...
        cat_agg = fdf.groupby("Category").agg(
            ...
            Avg_K=("Avg Pkg (LPA)","mean"),
            Count=("College Name","count"),
            Max_K=("Avg Pkg (LPA)","max")
        ).reset_index()

        c1, c2 = st.columns(2)
        with c1:
            fig7 = px.scatter(cat_agg, x="Avg_K", y="Avg_P",
        ...
        with c2:
            fig8 = go.Figure()
            fig8.add_trace(go.Bar(name="Avg Placement %",  x=cat_agg["Category"],
                                  y=cat_agg["Avg_P"].round(1), marker_color="#6366f1",
                                  text=cat_agg["Avg_P"].round(1),
                                  texttemplate="%{text}%", textposition="outside",
                                  textfont=dict(color="#e2e8f0")))
            fig8.add_trace(go.Bar(name="Avg Package (LPA)",x=cat_agg["Category"],
                                  y=cat_agg["Avg_K"].round(1), marker_color="#06b6d4",
                                  text=cat_agg["Avg_K"].round(1),
                                  texttemplate="₹%{text}", textposition="outside",
                                  textfont=dict(color="#e2e8f0"), yaxis="y2"))
            fig8.update_layout(**BASE, barmode="group",
                               title="Category: Placement % & Avg Package",
                               yaxis2=dict(title="Package (LPA)", overlaying="y",
                                           side="right", tickfont=dict(color="#475569")))
...
# ═══════════════════════════════════════════════════════════════
    ...
    # Summary pills
    pills = "".join([
        f'<span style="display:inline-block;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:600;'
        f'background:rgba(99,102,241,0.1);border:1px solid rgba(99,102,241,0.3);color:#a5b4fc;margin:3px;">'
        f'{r["College Name"].split("(")[0].strip()} — {r["Tier"]} | {r["Avg Pkg Range"]} | {r["Placement %"]:.0f}% placed</span>'
    ...
    with tabs[1]:
        ...
        metrics_bar = [
            ...
            ("Avg Pkg (LPA)",        "Avg Package (LPA midpoint)", "₹%{text}"),
